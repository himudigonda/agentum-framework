...
from agentum import Agent, GoogleLLM, State, Workflow, search_web_tavily, transcribe_audio
load_dotenv()
...
class VoiceRequestState(State):
    ...
    transcription: str = ''
    research_result: str = ''
researcher = Agent(name='VoiceResearcher', system_prompt="You are a helpful assistant. You first understand a user's request by transcribing it, then you search the web to find an answer.", llm=GoogleLLM(api_key=os.getenv('GOOGLE_API_KEY'), model='gemini-2.5-flash-lite'), tools=[search_web_tavily, transcribe_audio])
voice_workflow = Workflow(name='Voice_Request_Workflow', state=VoiceRequestState)
voice_workflow.add_task(name='transcribe_request', tool=transcribe_audio, inputs={'audio_filepath': '{audio_filepath}', 'project_id': os.getenv('GOOGLE_CLOUD_PROJECT_ID')}, output_mapping={'transcription': 'output'})
voice_workflow.add_task(name='conduct_research', agent=researcher, instructions='Perform a web search for the following request: {transcription}', output_mapping={'research_result': 'output'})
...
if __name__ == '__main__':
    ...
    if not os.path.exists(audio_file):
        ...
        print('curl -o request.wav "https://storage.googleapis.com/agentum-examples/request.wav"')
    else:
