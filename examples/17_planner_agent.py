...
researcher = Agent(name='Researcher', system_prompt='You are an expert market researcher. You write detailed, factual text.', llm=llm)
writer = Agent(name='Writer', system_prompt='You are an expert report writer. You take text and integrate it into a formal report.', llm=llm)
planner = Agent(name='Planner', system_prompt="You are the ultimate workflow orchestrator. Your job is to decide the single, next task to execute to complete the user's GOAL.\n    \n    You must output ONLY one of the following task names or 'DONE' if the GOAL is complete:\n    - 'write_introduction'\n    - 'write_body'\n    - 'write_conclusion'\n    - 'assemble_report'\n    - 'DONE'\n    \n    Current State of Report:\n    - Introduction: {introduction}\n    - Body: {body_section}\n    - Conclusion: {conclusion}\n    \n    GOAL: {goal}\n    ", llm=llm)
planner_workflow = Workflow(name='Dynamic_Planner_Workflow', state=ResearchPlanState)
planner_workflow.add_task(name='write_introduction', agent=researcher, instructions='Write a comprehensive 2-paragraph introduction for the report on the GOAL: {goal}', output_mapping={'introduction': 'output'})
planner_workflow.add_task(name='write_body', agent=researcher, instructions='Write the main 3-paragraph body section for the report on the GOAL: {goal}', output_mapping={'body_section': 'output'})
planner_workflow.add_task(name='write_conclusion', agent=researcher, instructions='Write a concise conclusion for the report on the GOAL: {goal}', output_mapping={'conclusion': 'output'})
planner_workflow.add_task(name='assemble_report', agent=writer, instructions='Assemble the final professional report from these parts:\n    Introduction: {introduction}\n    Body: {body_section}\n    Conclusion: {conclusion}\n    ', output_mapping={'final_report': 'output'})
planner_workflow.add_task(name='planning_step', agent=planner, instructions='Decide the next task based on the current report state to achieve the GOAL: {goal}', output_mapping={'plan': 'output'})
...
def planner_decides_next_step(state: ResearchPlanState) -> str:
    ...
    if next_task == 'DONE':
        return 'DONE'
    return next_task
planner_workflow.add_conditional_edges(source='planning_step', path=planner_decides_next_step, paths={'write_introduction': 'write_introduction', 'write_body': 'write_body', 'write_conclusion': 'write_conclusion', 'assemble_report': 'assemble_report', 'DONE': planner_workflow.END})
