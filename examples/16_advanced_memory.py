...
class ChatState(State):
    request: str
    response: str = ''
personal_assistant = Agent(name='PersonalAssistant', system_prompt='You are a personal assistant. You must use your memory to recall past information about the user.', llm=GoogleLLM(api_key=settings.GOOGLE_API_KEY, model='gemini-2.5-flash-lite'), memory=VectorStoreMemory())
memory_workflow = Workflow(name='Advanced_Memory_Assistant', state=ChatState)
memory_workflow.add_task(name='chat_task', agent=personal_assistant, instructions='User: {request}', output_mapping={'response': 'output'})
...
async def main():
    ...
    if 'python developer' in result_3['response'].lower() and 'agentum' in result_3['response'].lower():
