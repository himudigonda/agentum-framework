...
analyst = Agent(name='FinancialAnalyst', system_prompt='You are an expert financial analyst. You must answer questions using only the information found by your tools.', llm=GoogleLLM(api_key=settings.GOOGLE_API_KEY, model='gemini-2.5-flash-lite'), tools=[vector_search_tool])
rag_workflow = Workflow(name='RAG_Analyst', state=AnalystState)
rag_workflow.add_task(name='answer_question', agent=analyst, instructions="\n    Use your tools to find information to answer the user's question.\n    \n    User Question: {question}\n    ", output_mapping={'answer': 'output'})
