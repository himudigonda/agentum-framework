...
class ResearchReportState(State):
    ...
    report_filepath: str = ''
    save_status: str = ''
researcher_agent = Agent(name='Researcher', system_prompt='You are an expert researcher. Your job is to use the search tool to find comprehensive, up-to-date information on a topic and then synthesize it into a coherent summary.', llm=GoogleLLM(api_key=os.getenv('GOOGLE_API_KEY'), model='gemini-2.5-flash-lite'), tools=[search_web_tavily])
writer_agent = Agent(name='Writer', system_prompt='You are a professional writer. Your job is to format research data into a well-structured markdown report.', llm=GoogleLLM(api_key=os.getenv('GOOGLE_API_KEY'), model='gemini-2.5-flash-lite'))
pro_research_workflow = Workflow(name='Professional_Research_Pipeline', state=ResearchReportState)
pro_research_workflow.add_task(name='conduct_research', agent=researcher_agent, instructions='Perform a thorough web search and synthesize the findings for the topic: {topic}', output_mapping={'research_data': 'output'})
pro_research_workflow.add_task(name='write_report', agent=writer_agent, instructions='\n    Format the following research data into a clean, well-structured markdown report.\n    The report should have a title, an introduction, and sections for key findings.\n    \n    Research Data:\n    {research_data}\n    ', output_mapping={'report': 'output'})
pro_research_workflow.add_task(name='save_report_to_file', tool=write_file, inputs={'filepath': '{report_filepath}', 'content': '{report}'}, output_mapping={'save_status': 'output'})
...
if __name__ == '__main__':
    ...
    initial_state = {'topic': topic, 'report_filepath': f"./{topic.replace(' ', '_')}_report.md"}

    @pro_research_workflow.on('agent_tool_call')
    async def log_tool_call(tool_name: str, tool_args: dict):
        print(f"🔧 Agent is calling a REAL tool: {tool_name} with query: '{tool_args.get('query')}'")
