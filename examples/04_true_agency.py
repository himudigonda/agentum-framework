...
class TravelState(State):
    request: str
    plan: str = ''
travel_agent = Agent(name='TravelPlanner', system_prompt='You are a helpful travel assistant. You create simple, bulleted travel plans. When creating packing recommendations, always check the weather for the destination to provide accurate advice.', llm=GoogleLLM(api_key=os.getenv('GOOGLE_API_KEY'), model='gemini-2.5-flash-lite'), tools=[get_weather])
...
if __name__ == '__main__':
    initial_state = {'request': "I'm thinking of visiting San Francisco. What should I pack?"}
