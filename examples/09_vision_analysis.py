...
class VisionState(State):
    ...
    image_url: str
    description: str = ''
visual_analyst = Agent(name='VisualAnalyst', system_prompt='You are an expert at analyzing images and describing what you see in detail. You answer questions based on the provided image.', llm=GoogleLLM(api_key=os.getenv('GOOGLE_API_KEY'), model='gemini-2.5-flash-lite'))
vision_workflow = Workflow(name='Image_Analysis_Workflow', state=VisionState)
vision_workflow.add_task(name='analyze_image', agent=visual_analyst, instructions='{question}', output_mapping={'description': 'output'})
vision_workflow.set_entry_point('analyze_image')
vision_workflow.add_edge('analyze_image', vision_workflow.END)
if __name__ == '__main__':
    initial_state = {'question': 'Describe this scene. What city is it, and what is the overall mood?', 'image_url': 'https://images.unsplash.com/photo-1542051841857-5f90071e7989?q=80&w=2070&auto=format&fit=crop'}
    ...
    if 'tokyo' in final_state['description'].lower() and 'skyline' in final_state['description'].lower():
