...
def create_knowledge_base() -> KnowledgeBase:
    ...
    sample_documents = ['Artificial Intelligence is transforming industries worldwide.', 'Machine learning algorithms can process vast amounts of data.', 'Natural language processing enables computers to understand human language.', 'Computer vision allows machines to interpret visual information.', 'Deep learning networks can recognize patterns in complex data.']
...
def create_advanced_workflow() -> Workflow:
    ...
    summarizer = Agent(name='Summarizer', system_prompt='You are a professional summarizer who creates clear, concise summaries.', llm=anthropic_llm, tools=[format_summary], memory=ConversationMemory(), max_retries=3)
    workflow = Workflow(name='Advanced_Demo_Workflow', state=AdvancedState)
    workflow.add_task(name='research_task', agent=researcher, instructions='Research and gather information about: {user_query}', output_mapping={'research_result': 'output'})
    workflow.add_task(name='knowledge_search', agent=researcher, instructions='Search the knowledge base for: {user_query}', output_mapping={'knowledge_search_result': 'output'})
    workflow.add_task(name='analysis_task', agent=analyst, instructions='Analyze the research results: {research_result}', output_mapping={'analysis_result': 'output'})
    workflow.add_task(name='visual_analysis', agent=visual_analyst, instructions='Analyze this image and describe what you see: {user_query}', output_mapping={'analysis_result': 'output'})
    workflow.add_task(name='summarization_task', agent=summarizer, instructions='Create a comprehensive summary combining: Research: {research_result}, Analysis: {analysis_result}, Knowledge: {knowledge_search_result}', output_mapping={'final_summary': 'output'})
...
def run_text_only_demo():
    ...
    initial_state = {'user_query': 'What are the key trends in artificial intelligence?', 'image_url': ''}
...
def run_vision_demo():
    ...
    workflow = create_advanced_workflow()
    initial_state = {'user_query': 'Describe this image and analyze what you see', 'image_url': 'https://images.unsplash.com/photo-1677442136019-21780ecad995?q=80&w=2070&auto=format&fit=crop'}
...
def run_provider_comparison():
    ...
    google_workflow.add_task(name='google_task', agent=google_agent, instructions=test_query, output_mapping={'final_summary': 'output'})
    google_workflow.set_entry_point('google_task')
    google_workflow.add_edge('google_task', google_workflow.END)
    anthropic_agent = Agent(name='AnthropicAgent', system_prompt='You are a helpful assistant.', llm=anthropic_llm)
    anthropic_workflow = Workflow(name='AnthropicTest', state=AdvancedState)
    anthropic_workflow.add_task(name='anthropic_task', agent=anthropic_agent, instructions=test_query, output_mapping={'final_summary': 'output'})
