...
class VoiceAssistantState(State):
    ...
    question_text: str = ''
    answer_text: str = ''
assistant_agent = Agent(name='HelpfulAssistant', system_prompt='You are a helpful and friendly assistant. You provide concise, clear answers to questions.', llm=GoogleLLM(api_key=settings.GOOGLE_API_KEY, model='gemini-2.5-flash-lite'))
voice_assistant_workflow = Workflow(name='Voice_Assistant_Workflow', state=VoiceAssistantState)
voice_assistant_workflow.add_task(name='transcribe_question', tool=transcribe_audio, inputs={'audio_filepath': '{input_audio_path}'}, output_mapping={'question_text': 'output'})
voice_assistant_workflow.add_task(name='generate_answer', agent=assistant_agent, instructions='Answer the following question: {question_text}', output_mapping={'answer_text': 'output'})
voice_assistant_workflow.add_task(name='speak_answer', tool=text_to_speech, inputs={'text_to_speak': '{answer_text}', 'output_filepath': '{output_audio_path}'})
...
if __name__ == '__main__':
    ...
    if not os.path.exists(input_file):
        print(f'Input audio file not found. Please run the command from example 11 to download it.')
    else:
        initial_state = {'input_audio_path': input_file, 'output_audio_path': output_file}
