...
voice_assistant_workflow.add_task(
    ...
    inputs={"audio_filepath": "{input_audio_path}"},
...
voice_assistant_workflow.add_task(
    name="speak_answer",
    tool=text_to_speech,
    inputs={"text_to_speak": "{answer_text}", "output_filepath": "{output_audio_path}"},
)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(input_file):
        print(
            "Input audio file not found. Please run the command from example 11 to download it."
        )
    else:
        initial_state = {
