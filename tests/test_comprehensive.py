...
class TestFramework:
    ...
    def __init__(self):
        if not settings.GOOGLE_API_KEY:
            pytest.skip('GOOGLE_API_KEY not found in settings, skipping comprehensive suite')
        if not settings.ANTHROPIC_API_KEY:
            pytest.skip('ANTHROPIC_API_KEY not found in settings, skipping comprehensive suite')
        try:
    ...
    def test_agent_creation(self):
        ...
        if self.google_llm:
            google_agent = Agent(name='GoogleAgent', system_prompt='You are a helpful assistant.', llm=self.google_llm)
            assert google_agent.name == 'GoogleAgent'
            assert google_agent.llm == self.google_llm
            print('✅ Google agent created successfully')
        if self.anthropic_llm:
            anthropic_agent = Agent(name='AnthropicAgent', system_prompt='You are a helpful assistant.', llm=self.anthropic_llm)
    ...
    def test_workflow_creation(self):
        ...
        workflow.add_task(name='test_task', agent=agent, instructions='Process this input: {input_text}', output_mapping={'result': 'output'})
    ...
    def test_tool_integration(self):
        ...
        if not llm:
            ...
            return
        agent = Agent(name='ToolAgent', system_prompt='You are a tool-using agent.', llm=llm, tools=[test_tool])
    ...
    def test_memory_integration(self):
        ...
        agent = Agent(name='MemoryAgent', system_prompt='You are a memory-enabled agent.', llm=llm, memory=memory)
    ...
    def test_vision_state_detection(self):
        ...
        assert not (hasattr(state_no_image, 'image_url') and getattr(state_no_image, 'image_url'))
        state_with_image = TestState(input_text='Hello', image_url='https://example.com/image.jpg')
        assert hasattr(state_with_image, 'image_url') and getattr(state_with_image, 'image_url')
    ...
    def test_multi_agent_workflow(self):
        ...
        if not llm1 or not llm2:
            ...
            return
        agent1 = Agent(name='Agent1', system_prompt='You are the first agent.', llm=llm1)
        agent2 = Agent(name='Agent2', system_prompt='You are the second agent.', llm=llm2)
        workflow.add_task(name='task1', agent=agent1, instructions='Process: {input_text}', output_mapping={'intermediate_result': 'output'})
        workflow.add_task(name='task2', agent=agent2, instructions='Continue processing: {intermediate_result}', output_mapping={'result': 'output'})
    ...
    def test_error_handling(self):
        ...
        workflow.add_task(name='error_task', agent=agent, instructions='Process: {nonexistent_key}', output_mapping={'result': 'output'})
