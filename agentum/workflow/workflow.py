...
class Workflow:
    ...
    def __init__(self, name: str, state: Type[State], persistence: Optional[str]=None):
        self.name = name
    ...
    def add_task(self, name: str, agent: Optional[Any]=None, tool: Optional[Callable]=None, instructions: Optional[str]=None, inputs: Optional[Dict[str, Any]]=None, output_mapping: Optional[Dict[str, str]]=None):
        if name in self.tasks:
            raise TaskConfigurationError(f"Task '{name}' already exists.")
        if not agent and (not tool):
            raise TaskConfigurationError(f"Task '{name}' must have either an agent or a tool.")
        if agent and tool:
            raise TaskConfigurationError(f"Task '{name}' cannot have both an agent and a tool.")
        if agent and (not instructions):
            raise TaskConfigurationError(f"Agent task '{name}' must have instructions.")
        if tool and (not inputs):
            console.print(f"[yellow]Warning: Tool task '{name}' has no input mapping.[/yellow]")
        self.tasks[name] = {'agent': agent, 'tool': tool, 'instructions': instructions, 'inputs': inputs, 'output_mapping': output_mapping}
    ...
    def set_entry_point(self, task_name: str):
        if task_name not in self.tasks:
            raise WorkflowDefinitionError(f"Entry point task '{task_name}' does not exist.")
        self.entry_point = task_name
        console.print(f'  - Entry point set to: [cyan]{task_name}[/cyan]')

    def _compile(self):
        if not self._compiled_graph:
            console.print('\n🔧 [bold]Compiling workflow into an executable graph...[/bold]', style='blue')
    ...
    async def astream(self, initial_state: Dict, thread_id: Optional[str]=None) -> AsyncGenerator[Dict[str, Any], None]:
