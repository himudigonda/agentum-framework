...
def _is_safe_path(path_str: str) -> bool:
    return not path_str.startswith('/') and '..' not in path_str and (not path_str.startswith('~')) and (not path_str.startswith('\\'))
...
class GraphCompiler:
    ...
    def _create_agent_node(self, task_name: str, task_details: Dict):
        ...
        if agent.tools:
            console.print(f'    - Binding {len(agent.tools)} tools to LLM: {[t.__name__ for t in agent.tools]}')
        ...
        async def agent_node(state: State) -> Dict[str, Any]:
            ...
            console.print(f'  Executing Agent Task: [bold magenta]{task_name}[/bold magenta]')
            try:
                formatted_instructions = _safe_format(instructions_template, state.model_dump())
            except Exception as e:
                raise StateValidationError(f"Missing state key '{e}' required by task '{task_name}' instructions template.")
            ...
            if isinstance(agent.llm, GoogleLLM):
                if hasattr(state, 'image_path') and getattr(state, 'image_path'):
                    ...
                    if not _is_safe_path(image_path_str):
                        console.print(f'[bold red]SECURITY ERROR: Path traversal detected in {image_path_str}. Skipping image.[/bold red]')
                    else:
                        image_path = Path(image_path_str)
                        if image_path.exists():
                            console.print(f'    - Attaching local image for analysis: {image_path_str}')
                            mime_type, _ = mimetypes.guess_type(image_path)
                            if mime_type:
                                with open(image_path, 'rb') as image_file:
                                    base64_image = base64.b64encode(image_file.read()).decode('utf-8')
                                message_content.append({'type': 'image_url', 'image_url': {'url': f'data:{mime_type};base64,{base64_image}'}})
                            else:
                                console.print(f'[yellow]Warning: Could not determine MIME type for {image_path_str}. Skipping image.[/yellow]')
                        else:
                            console.print(f'[yellow]Warning: Image file not found at {image_path_str}. Skipping image.[/yellow]')
                ...
                console.print('[yellow]Warning: Agent LLM does not support multi-modal input. Image logic skipped.[/yellow]')
                ...
                try:
                    while True:
                        ...
                        if not response.tool_calls:
                            ...
                            console.print(Panel(agent_output, title=f'[bold blue]{agent.name}[/bold blue] Output', border_style='blue', padding=(1, 2)))
                            break
                        console.print(f"    - Agent '{agent.name}' wants to call tools: {[tc['name'] for tc in response.tool_calls]}")
                        messages.append(response)
                        for tool_call in response.tool_calls:
                            await self.workflow._emit('agent_tool_call', tool_name=tool_call['name'], tool_args=tool_call['args'])
                            tool_func = next((t for t in agent.tools if t.__name__ == tool_call['name']), None)
                            if not tool_func:
                            ...
                            try:
                                ...
                                console.print(Panel(tool_output, title=f"[bold green]Tool '{tool_call['name']}'[/bold green] Result", border_style='green', padding=(1, 2)))
                                await self.workflow._emit('agent_tool_result', tool_name=tool_call['name'], result=result)
                                messages.append(ToolMessage(content=str(result), tool_call_id=tool_call['id']))
                            except Exception as e:
                                error_message = f"Error executing tool '{tool_call['name']}': {e}"
                                console.print(f'[bold red]    - {error_message}[/bold red]')
                                messages.append(ToolMessage(content=error_message, tool_call_id=tool_call['id']))
                    break
                except Exception as e:
                    console.print(f'[bold yellow]  - Attempt {attempt + 1}/{agent.max_retries} failed: {e}[/bold yellow]')
            ...
            await self.workflow._emit('agent_end', agent_name=agent.name, final_response=final_content)
            state_update = {}
            if output_mapping:
                state_update = {state_key: final_content for state_key, response_key in output_mapping.items()}
            await self.workflow._emit('task_finish', task_name=task_name, state_update=state_update)
            return state_update
        return agent_node
    ...
    def _create_tool_node(self, task_name: str, task_details: Dict):
        ...
        async def tool_node(state: State) -> Dict[str, Any]:
            ...
            try:
                resolved_inputs = {key: _safe_format(template, state.model_dump()) for key, template in input_mapping.items()}
            except Exception as e:
                raise StateValidationError(f"Error resolving inputs for tool '{task_name}': {e}")
            if inspect.iscoroutinefunction(tool_func):
                result = await tool_func(**resolved_inputs)
            else:
                result = await asyncio.to_thread(tool_func, **resolved_inputs)
            tool_output = str(result).strip()
            console.print(Panel(tool_output, title=f"[bold cyan]Tool '{task_name}'[/bold cyan] Result", border_style='cyan', padding=(1, 2)))
            state_update = {}
            if output_mapping:
                state_update = {state_key: result for state_key, response_key in output_mapping.items()}
            await self.workflow._emit('task_finish', task_name=task_name, state_update=state_update)
            return state_update
        return tool_node
