...
class Evaluator:
    ...
    async def evaluate(self, state: Dict[str, Any]) -> Dict:
        ...
        response = await self.agent.llm.ainvoke(f'{self.agent.system_prompt}\n\n{prompt}')
        return {'evaluator': self.name, 'score': response.content}

class TestSuite:

    def __init__(self, workflow: Workflow, dataset: List[Dict], evaluators: List[Evaluator]):
    ...
    async def arun(self) -> List[Dict]:
        ...
        for i, initial_state in enumerate(self.dataset):
            ...
            results.append({'initial_state': initial_state, 'final_state': final_state, 'evaluations': evaluations})
        return results
