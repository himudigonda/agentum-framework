...
def make_layout() -> Layout:
    ...
    layout.split_column(Layout(name='header', size=3), Layout(name='log'), Layout(name='final_state', size=10))
    layout['header'].update(Panel(Text('🚀 Agentum Live Tracer', style='bold green', justify='center'), title='[bold magenta]STATUS[/bold magenta]', border_style='green', height=3))
    layout['final_state'].update(Panel(Text('Waiting for completion...', style='dim'), title='[bold yellow]Final State[/bold yellow]', border_style='yellow'))
    return layout

@app.command()
def run(script_path: str=typer.Argument(..., help='Path to the Python script containing the workflow'), initial_state: Optional[str]=typer.Option(None, '--state', '-s', help='Initial state as JSON string'), thread_id: Optional[str]=typer.Option(None, '--thread-id', '-t', help='Thread ID for state persistence'), stream: bool=typer.Option(False, '--stream', help='Stream workflow execution in real-time')):
    ...
    try:
        ...
        for attr_name in dir(module):
            ...
            if isinstance(attr, Workflow):
                ...
                break
        if not workflow:
            console.print(f"[red]Error: No Workflow instance found in '{script_path}'.[/red]")
            console.print('[yellow]Make sure your script defines a workflow variable.[/yellow]')
            raise typer.Exit(1)
        import json
        if initial_state:
            try:
...
async def _run_streaming(workflow: Workflow, state: dict, thread_id: Optional[str]):
    ...
    def log(message: str, style: str='white'):
        ...
        layout['log'].update(Panel(log_content, title='[bold]Workflow Log[/bold]', border_style='white'))
            ...
            async for event in workflow.astream(state, thread_id=thread_id):
                for node_name, node_output in event.items():
                    ...
                    if node_output:
                        ...
                        log(f"  ↳ [dim]Updated State Keys:[/dim] {', '.join(update_keys)}", 'dim')
                    pass
                    state.update(node_output)
            final_state = state
            log('🏁 Workflow finished.', 'bold green')
            final_state_syntax = Syntax(json.dumps(final_state, indent=2, default=str), 'json', theme='monokai', line_numbers=True)
            layout['final_state'].update(Panel(final_state_syntax, title='[bold green]Final State[/bold green]', border_style='green'))
...
@app.command()
def init(name: str=typer.Argument(..., help='Name of the workflow'), output_dir: str=typer.Option('.', '--output', '-o', help='Output directory')):
...
@app.command()
def validate(script_path: str=typer.Argument(..., help='Path to the Python script to validate')):
    script_file = Path(script_path)
    if not script_file.exists():
        console.print(f"[red]Error: Script '{script_path}' not found.[/red]")
        raise typer.Exit(1)
...
@app.command()
def graph(script_path: str=typer.Argument(..., help='Path to the Python script'), output_file: str=typer.Option('workflow_graph.png', '--output', '-o', help='Output file for the graph')):
    script_file = Path(script_path)
    if not script_file.exists():
        console.print(f"[red]Error: Script '{script_path}' not found.[/red]")
        raise typer.Exit(1)
