...
class SummaryMemory(BaseMemory):
    ...
    def load_messages(self) -> List[BaseMessage]:
        if self.summary:
            return [AIMessage(content=f'Summary of prior conversation: {self.summary}', role='system')]
        return []

    def save_messages(self, messages: List[BaseMessage]):
        self.history.extend(messages)
        new_lines = get_buffer_string(messages)
        chain = LLMChain(llm=self.llm, prompt=PromptTemplate.from_template(self.SUMMARIZER_PROMPT))
        self.summary = chain.invoke({'summary': self.summary, 'new_lines': new_lines}).get('text', '')

class VectorStoreMemory(BaseMemory):
    model_config = ConfigDict(arbitrary_types_allowed=True)
    vector_store: Chroma = Field(default_factory=lambda: Chroma(collection_name='vector_memory', embedding_function=get_embedding_function()))
    ...
    def load_messages(self) -> List[BaseMessage]:
        ...
        context_message = '\n'.join([f'Past Context: {doc.page_content}' for doc in relevant_docs])
        return [AIMessage(content=f'Relevant historical context:\n{context_message}', role='system')]

    def save_messages(self, messages: List[BaseMessage]):
        text_to_save = get_buffer_string(messages)
        self.vector_store.add_texts([text_to_save], metadatas=[{'timestamp': str(datetime.now())}])
        self.current_buffer = []
